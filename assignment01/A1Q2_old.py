...
# I want to know the 1, 10, 100, 1000th to 1million
...
def increment_v3(starting_number, m, diff):
    ...
    if len(starting_number) == 1:
        ...
        return _, m, 0
    # Check the diff
    print("Remaining diff is ", diff)
    print("Starting number is ", starting_number)
    print("Next number is ", next_number)
    if diff == 0:
        _ = starting_number[:-len(next_number)] + str(next_number) 
        return _, m, diff
...
def AQQ2():
    ...
    starting_idxs = generate_smartly(6,4)
    print(starting_idxs)
    closest_idx = get_closest_index(starting_idxs)
    print(closest_idx)
